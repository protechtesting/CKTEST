...
public class WPSignUpSignInPopUp extends Seleniumbase{
	...
	public WPSignUpSignInPopUp(WebDriver driver, ExtentTest logger){
		...
		/*
		 * if(Utils.verifyElementPresence(lnkSignUp)) {
		 * 
		 * //|| Utils.verifyElementPresence(txtBxOTP) Log.info(pass); reportStep(pass,
		 * "PASS"); }else { Log.error(fail); reportStep(fail, "FAIL"); }
		 */
	...
	public WPSignUpSignInPopUp verifyEmailPlaceHolderValue() {
		try {
			...
			if(Utils.verifyPlaceHolderValue(txtBxEmail, "Enter Email"));
	...
	public WPSignUpSignInPopUp enterOtp(String otp) {
		if(Utils.sendKeys(txtBxOTP, otp)) {
			Log.info("Entered Successfully OTP-- "+otp+" in OTP Text Box during Sign Up");
			reportStep("Entered Successfully OTP-- "+otp+" in OTP Text Box during Sign Up","PASS");
		}else {
	...
	public WPHomePage hitICICILoginwithFedIDAndChannelfromHomepage(String FedID, String Channel) {
	...
	public WPHomePage hitICICILoginonlywithFedIDHomePage(String FedID) {
		Log.info("Launching ICICI with the FedID "+FedID);
		reportStep("Launching ICICI with the "+FedID+"number", "INFO");
	...
	public WPStorePage hitICICILoginwithFedIDAndChannelfromStorepage(String FedID, String Channel) {
		Log.info("Launching ICICI with the FedID "+FedID);
		reportStep("Launching ICICI with the "+FedID+"number", "INFO");
		Utils.iciciLoginwithFedIDAndChannel(driver, FedID, Channel);
		Log.info("Successfully launched url with FEDID :" +FedID+ " & Channel :"+Channel);
	...
	public WPStorePage hitICICILoginonlywithFedIDStorePage(String FedID) {
		Log.info("Launching ICICI with the FedID "+FedID);
		reportStep("Launching ICICI with the "+FedID+"number", "INFO");
		Utils.iciciLoginwithFedID(driver,txtreference, FedID);
		Log.info("Successfully launched url with FEDID :" +FedID);
	...
	public WPStorePage enterReference(String Ref) {
	...
	{	
		...
		verifyDisplayed(driver,"Password field",txtBxPassword);		
		verifyDisplayed(driver,"Sign In button",btnSignInWithEmail);
		verifyDisplayed(driver,"Forget password link",lnkForgotPassword);
		verifyDisplayed(driver,"Close Icon",btnClosePopUp);
