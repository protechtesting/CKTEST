...
public class Utils extends TestSetUp{
	...
	private static final XMLReader objConfigReader = new XMLReader(System.getProperty("user.dir") + "/input/configurationURLs.xml");
	...
	public static String getConfigData(int index,String key){
		try {
			Log.info("Fetching Test Data for the test");
			List<Hashtable<String, String>> testDataHashTable = objConfigReader.getDataAsList("ConfigurationUrls");
			testData= testDataHashTable.get(index).get(key).trim();
			Log.info("Successfully fetched Test Data for the test");
			return testData;
		}catch(Exception e) {
	...
	public static void iciciLoginwithFedIDAndChannel(WebDriver driver,String FedID, String channelName) {
		String baseURL="http://mychaddi.in/auth?FedID=";
		String iciciloginURL=baseURL+FedID+"&channel="+channelName;
		
		 openNewTab(driver); Log.info("Opened New Tab");
		 switchToNewlyOpenWindow(driver);
		 presence=verifyElementPresence(WPCreationOfDataByHittingURLS.
		 lblSuccessMessage); if(presence) {
		 Log.info("\"Success\" Message is present"); } else {
		  Log.error("\"Success\" Message is not present"); }
		 Log.info("Successfully switched to new window");
		 
		driver.get(iciciloginURL);
		Log.info("Successfully added money to account");
	...
	public static void iciciLoginwithFedIDAndChannelURL(WebDriver driver,String FedID, String channelName, String URL) {
		String baseURL="http://mychaddi.in/auth?FedID=";
		String iciciloginURL=baseURL+FedID+"&channel="+channelName+URL;
	...
	public static void iciciLoginwithFedID(WebDriver driver,WebElement ele,String FedID) {
		String baseURL="http://mychaddi.in/auth?FedID=";
		String iciciloginURL=baseURL+FedID;
		openNewTab(driver);
		Log.info("Opened New Tab");
		switchToNewlyOpenWindow(driver);
		presence=verifyElementPresence(WPCreationOfDataByHittingURLS.lblSuccessMessage);
		...
		sendKeysAndPressEnter(ele, FedID);
	...
	public static void iciciLoginwithFedIDAndChannelRedirectionURL(WebDriver driver,String FedID, String channelName, String url) {
		String baseURL="http://mychaddi.in/auth?FedID=";
		String iciciloginURL=baseURL+FedID+"&channel="+channelName;
	...
	public void launchLogoutURL(WebDriver driver)
	{
		driver.get("https://mychaddi.in/logout");
		driver.close();
	}
	
	
	public static Boolean verifycontainsUserID(WebDriver driver, WebElement element)
	{
